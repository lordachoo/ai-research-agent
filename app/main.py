...
def parse_args():
    ...
    web_parser = subparsers.add_parser("web", help="Start the FastAPI HTML web interface")
    ...
    web_parser.add_argument("--log-level", default="info", help="Log level for server")
...
def main():
    ...
    if args.command == "query":
        ...
        from app.ui.web_launcher import launch_web_ui
        
        print(f"Starting FastAPI HTML web interface on {args.host}:{args.port}")
        print(f"Once started, you can access the UI at: http://{args.host}:{args.port}")
        
        # Launch FastAPI HTML UI
        launch_web_ui(
            host=args.host,
            port=args.port,
            reload=args.reload,
